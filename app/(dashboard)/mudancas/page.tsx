...
import { MUDANCAS, type Mudanca } from "@/lib/mock-data";
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Toda mudança feita no painel + impacto medido após 7/14/21 dias"
      />
      ...
      <div className="relative">
        ...
        <div className="space-y-4">
          {MUDANCAS.map((m, idx) => {
            const cfg = VEREDICTO_CONFIG[m.veredicto];
            const Icon = cfg.icon;
            return (
              <motion.div
                key={m.id}
                    ...
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          Por <span className="font-medium">{m.feita_por}</span> ·{" "}
                          {formatDateTime(m.feita_em)}
                        </p>
                      </div>
                      <Badge variant={cfg.badge}>
                        {cfg.label}{" "}
                        {m.veredicto !== "aguardando" && `· ${m.impacto_dias}d`}
                      </Badge>
                    </div>
