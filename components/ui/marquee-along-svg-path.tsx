...
// Renders a single child positioned along the SVG path. Extracted as a
// component so the motion hooks run at the top level (rules of hooks).
const MarqueeItem = ({
  children,
  baseOffset,
  itemIndex,
  ...
  const currentOffsetDistance = useTransform(itemOffset, (value) =>
    parseFloat(value)
  );
  ...
  // Interpolated CSS variables, created imperatively (motionValue is not a
  // hook) so a dynamic list of interpolations stays hook-safe.
  const cssVariables = React.useMemo(
    () =>
      Object.fromEntries(
        cssVariableInterpolation.map(({ property, from }) => [
          property,
          motionValue(from),
        ])
      ),
  ...
  useEffect(() => {
    ...
    const update = (value: number) => {
      for (const { property, from, to } of cssVariableInterpolation) {
        cssVariables[property]?.set(transform(value, [0, 100], [from, to]));
      }
    };
    update(currentOffsetDistance.get());
    return currentOffsetDistance.on("change", update);
  }, [cssVariableInterpolation, cssVariables, currentOffsetDistance]);
...
const MarqueeAlongSvgPath = ({
  ...
  // Stable ID for the path (SSR-safe, unlike Math.random)
  const reactId = useId();
  ...
  const handleItemHoverChange = useCallback((hovered: boolean) => {
