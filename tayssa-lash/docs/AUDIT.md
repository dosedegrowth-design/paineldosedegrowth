...
## Entregue nesta sessão (16/set/2026) — tudo na `main`

| Fase | O que mudou | Commit |
|---|---|---|
| A · Cadastro + aprovação | `/cadastro` em 3 passos → `users.status = pending` → a Tayssa aprova/recusa na ficha (com VIP opcional e boas-vindas no WhatsApp); `/aguardando`; login diz "seu pedido está com a Tayssa"; `settings.signup` (aberto + mensagem) | `fb8419b` |
| B · Imagens | bucket `tayssa-fotos` + `vip.photos`; `/admin/fotos` (subir com estilo/volume, legenda, alt, destaque, arquivar, apagar); `assignPhotos()` distribui para herói, sequência, detalhes, serviços e cartão; sem foto → campo tonal | `b8d7802` |
| C · Cartão físico | proporção de cartão de crédito, frente com as fotos em sequência lenta + chip + nome, giro 3D real com mola (faces separadas 1px em Z), verso com as 8 posições e a raspadinha; inclinação no desktop; reduced-motion | `609e81c` |
| D · Home da cliente | anel de progresso até o próximo marco, contadores, "Sua jornada" (o que vem · hoje · o que foi) — `lib/tayssa/timeline.ts` puro e testado | `1926477` |
| E · Admin operacional | `/admin/agenda` (confirmar com recado, recusar/cancelar com motivo, realizado → visita confirmada + carimbo via `lib/tayssa/visits.ts`, não compareceu); agenda de hoje e pedidos na visão geral; configurações de Agenda, Cartão e Cadastro; a cliente vê o recado/motivo | `c8d8f5f` |
| F · Estados e movimento | `loading.tsx` e `error.tsx` no app e no admin; entrada de tela por rota; momento do marco (benefício alcançado/liberado nas últimas 72h, dispensável no aparelho); reduced-motion em tudo | `cf8c7be` |
| G · Gates + espelho | tsc 0 erros, eslint limpo no escopo Tayssa, testes puros verdes, `next build`; `tayssa-lash/` re-espelhado a partir do painel (script mecânico) | este commit |
...
- Fotos reais: o sistema está pronto e vazio — a Tayssa sobe pelo `/admin/fotos`.
- O upload real no Storage de produção foi testado só até onde dá sem foto real; o caminho é o mesmo do mock (bucket público criado pela migration).
- O eslint do repositório inteiro tem erros antigos fora do Tayssa (dashboard DDG, LP André, scripts de skills) — não tocados aqui.
